...
def app():
    ...
    with tab1:
        ...
        # ------------------------- RESULTS (FULL WIDTH) -------------------------
        # This code is now OUTSIDE the columns, so it spans the full width
        if st.session_state.results:
            ...
            # NOW we use columns for the result metrics
    ...
    with tab1:
        ...
        if st.session_state.results:
            ...
            with st.expander("View Detailed Step-by-Step Derivation (2 Methods)", expanded=True):
                ...
                st.latex(rf"\sigma' = z \cdot \gamma'_{{eff}} = {results['depth_A_soil']:.2f} \cdot {results['gamma_effective']:.2f} = \mathbf{{{results['sigma_prime_2']:.2f} \text{{ kPa}neck_xs
    # =================================================================
    # TAB 2: PERMEABILITY
    # =================================================================
    with tab2:
