...
# === CRITICAL CHANGE: Indent the RESULTS block so it is inside the tab3 block ===
        # By keeping it at this indentation level (same as col_setup), it takes the FULL WIDTH.
        if solve_clicked:
...
"""
            ...
            with st.expander("Detailed Calculation Log", expanded=True):
                ...
                stress_log = [
                    ...
                    f"**Pore Pressure ($u$):** $(h_{{target}} - z_{{elev}}) \\cdot \\gamma_w = ({h_target:.4f} - {z_elev:.2f}) \\cdot {gamma_w_val} = {u_target:.2f}$ kPa",
